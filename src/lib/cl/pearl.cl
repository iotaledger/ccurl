...
#pragma unroll
	for(i = 0; i < 3; i++) {
		...
		states[midLow + j] = states[j + STATE_LENGTH]; //midstatehigh
	}
	for (i = gid; i-- > 0; ) {
...
__kernel void transform (
	...
	size_t i, j,round;
	__private size_t id = my_id*3;
	__private size_t gid = get_global_id(0) ;// get_global_offset(0);
	__private size_t midLow = gid*STATE_LENGTH*4,
	midHigh = midLow + STATE_LENGTH,
       	low = midHigh + STATE_LENGTH, 
	high = low + STATE_LENGTH;
...
//
//   (((trit_t)(states[low + j] >> i)) & 1) != 
//   (((trit_t)(states[high + j] >> i)) & 1)) {
...
//trits[j] = ((((trit_t)(states[midLow + j] >> *bitIndex)) & 1) == 0) ? 1 : (((((trit_t)(states[midHigh + j] >> *bitIndex)) & 1) == 0) ? -1 : 0);
...
#pragma unroll
		for (k = 0; k < 3; k++) {
			j = id + k;
			if (!skip && j >= HASH_LENGTH - *minWeightMagnitude && 
					((trit_t)states[low + j] & (1 << i)) != 
					((trit_t)states[high + j] & (1 << i))) {
				skip = true;
			}
...
#pragma unroll
	for(i = 0; i < sizeof(size_t); i++) { 
		if(*found != 0) return;
		cl_transform(states, indices);
		j = cl_check(minWeightMagnitude, states);
		if (j != -1){
			if(id == 0) {
				*found |= *mask;
				*bitIndex = j;
