...
void *pearcl_find(void *data) {
	...
	check_clerror(
			clEnqueueWriteBuffer(pdcl->cl.clcmdq[thread->index],
				...
				pdcl->cl.kernel.buffer[2].size, &(thread->min_weight_magnitude), 0, 
				NULL, NULL),
	...
	check_clerror(
			clEnqueueReadBuffer(pdcl->cl.clcmdq[thread->index],
				pdcl->cl.buffers[thread->index][3], CL_TRUE, 0, sizeof(char),
...
bool pearcl_search(
	...
	pdcl->cl.kernel.num_buffers = 5; 
	pdcl->cl.kernel.buffer[0] = (BufferInfo){sizeof(trit_t)*HASH_LENGTH, CL_MEM_WRITE_ONLY};  // trit_hash //
	pdcl->cl.kernel.buffer[1] = (BufferInfo){sizeof(trit_t)*STATE_LENGTH*4, CL_MEM_READ_WRITE, 2}; // states array  //
	pdcl->cl.kernel.buffer[2] = (BufferInfo){sizeof(size_t), CL_MEM_READ_ONLY};                		// minweightmagnitude //
	pdcl->cl.kernel.buffer[3] = (BufferInfo){sizeof(char), CL_MEM_READ_WRITE};                 		// found //
	pdcl->cl.kernel.buffer[4] = (BufferInfo){sizeof(trit_t), CL_MEM_READ_WRITE, 2};           // nonce_probe //
